...
Pour les fichiers de données de plusieurs mois, il est possible de faire une prévisualisation des données avec une résolution maximale d'une seconde. Cela permet d'avoir une vue d'emble des données rapidement, ainsi que de localiser les zones d'activités, sans passer par la phase de recombinaison des fichiers TPL et ACC. Il peux être envisagé de rajouter une option pour exporter ces zones d'activités.

### Exemple de prévisualisation
```
filecsv"~/rtoolbox/wacu161.txt"
fileacccsv="~/rtoolbox/wacu161_ACC.txt"
filewacuh5="~/rtoolbox/wacu161.h5"
rblt::wacupre2h5(filecsv, fileacccsv,fileh)
#visualisation
ll=LoggerList$new()
...
#lors du traitement des données, l'utilitaire affichera les informations rtc_period,rtc_ticks et start_at qu'il faudra repporter dans le fonction wacu2h5 sous R
rtc_period:1
rtc_ticks:50
